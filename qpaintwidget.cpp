...
QPaintWidget::QPaintWidget(Surface *s_, QWidget *parent) :
    QWidget(parent), timer(this), s(s_), s2d(s_->getStates()), stage(0)
{
 ...
 //   std::cout << "draw start" << std::endl;
  //  g_mtx.lock();
    ...
    //    std::cout << "tan=" << tan30 << std::endl;
        size_t size0 = s2d.size(), size1 = s2d[0].size();
        qreal max_x = (size1 /*- 1*/) * POINT_GAP;
        for (size_t i = 0; i < size0; i++)
        {
            for (size_t j = 0; j < size1; j++)
            {
                Surface::host_state current_state = s2d[i][j];
...
#if 0
void QPaintWidget::drawSurface2()
{
 //   std::cout << "draw start" << std::endl;
  //  g_mtx.lock();
    //    std::cout << "in paint event" << std::endl;
...
//    drawSurface();
    if (/*g_iterations % 1000 == 0 || g_iterations == 1*/true)
//    if (stage % 1000 == 0 || stage == 1)
    {
